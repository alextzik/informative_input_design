...
def compute_next_input(theta_est: np.ndarray,
    ...
    Sigma_obs = torch.tensor(Sigma_obs, dtype=torch.float32)
    
    # Inverse of Sigma_obs
    inv_Sigma_obs = torch.inverse(Sigma_obs)
    ...
    for iteration in range(num_iterations):
        ...
        loss = -torch.logdet(
            const + model_derivative_matrix_tensor(x, theta_est).T @ inv_Sigma_obs @ model_derivative_matrix_tensor(x, theta_est)
